...
def operation_sub_model(transformation, data):
    m = pyo.ConcreteModel()
    ...
    m.bigM_plant_ava_cap = pyo.Param(m.plant, initialize = data['bigM_plant_ava_cap'], doc = 'Upper bounds of plant capacity')  
    
    # Bounds on plant
    def _bounds_cpa_rule(m, r, k, t):
        for k in m.plant:
            return (0, m.bigM_plant_ava_cap[k])
    
    def _bounds_cpo_rule(m, r, k, t, n, b):
        for k in m.plant:
            return (0, m.bigM_plant_ava_cap[k])
    
    # Bounds on parallel generator
    ...
    def investment_cost(m):
        return m.CAPEX == sum(m.DCF[t] * (sum(m.CCP[k,c] * m.yupi[r,k,c,t] for r,k in m.rg_plt_pn for c in m.size) + 
                                          ...
                                          sum(m.CCG[k,j,c] * m.ybin[r,k,j,c,t] for r,k in m.rg_plt_rn for j in m.prgen for c in m.size) + 
                                          sum(m.CCS * m.ysi[r,k,i,t] for r,k in m.rg_plt_rw for i in m.strge)) for t in m.year)
    ...
    def Operating_expenses(m):
        return m.OPEX == m.FOC + m.VOC + m.SUC + m.SDC + m.FUC + m.CEM
    ...
    def variable_operating_expenses(m):
        return m.VOC == sum(m.DCF[t] * m.OT[n,b] * (sum(m.VOC_P[k] * m.cpo[r,k,t,n,b] / 1000 for r,k in m.rg_plt) + 
    ...
    @m.Constraint()
    def startup_cost(m):
        return m.SUC == sum(m.DCF[t] * (sum(m.SUC_P[k] * m.usu[r,k,t,n,b] for r,k in m.rg_plt_dp) +
                                        sum(m.SUC_G[k,j] * m.psu[r,k,j,t,n,b] for r,k in m.rg_plt_rn for j in m.prgen))
                            for t in m.year for n in m.rpdn for b in m.sub)
     
    @m.Constraint()
    def shutdown_cost(m):
        return m.SDC == sum(m.DCF[t] * (sum(m.SDC_P[k] * m.usd[r,k,t,n,b] for r,k in m.rg_plt_dp) + 
                                        sum(m.SDC_G[k,j] * m.psd[r,k,j,t,n,b] for r,k in m.rg_plt_rn for j in m.prgen))
                            for t in m.year for n in m.rpdn for b in m.sub)
    
    @m.Constraint()
    def fuelcost(m):
        return m.FUC == sum(m.DCF[t] * m.FPC[k] * m.fs[r,k,t,n,b] for r,k in m.rg_plt_dp for t in m.year for n in m.rpdn for b in m.sub)
    
    @m.Constraint()
    def emission_cost(m):
        return m.CEM == sum(m.DCF[t] * m.CTX[t] * m.COE[k] * m.fs[r,k,t,n,b] for r,k in m.rg_plt for t in m.year for n in m.rpdn for b in m.sub)     
 
    @m.Constraint()
    def downtime_penalty(m):
        return m.DTP == sum(m.DCF[t] * m.beta * m.dt[r,k,t] for r,k in m.rg_plt for t in m.year)     

    @m.Constraint()
    def unmet_demand_penalty(m):
        return m.UMP == sum(m.DCF[t] * m.alpha * m.umd[t,n,b] for r,k in m.rg_plt for t in m.year for n in m.rpdn for b in m.sub)     
    ...
    def Z3_disjunct(outer_Z3, r, k, t):
        ...
        def z3w1(inner_Z3W1, n, b):
            ...
            def rf_z3w1(inner_Z3W1):
                return m.rf[r,k,t,n,b] == m.Prob_H3[1,k] + m.Prob_H3[2,k] + m.Prob_H3[3,k] + m.Prob_H3[4,k] + \
                       m.Prob_H3[5,k] + m.Prob_H3[6,k] + m.Prob_H3[7,k]
...
if __name__ == "__main__":
    d = dataset()
    m = operation_sub_model('gdp.hull', d)
